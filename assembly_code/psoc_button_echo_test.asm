; This program does the following when the button is pressed:
;   reads the ADC, sends the value to the PSOC
;   and then receives the value back from the PSOC and then
;   writes it to the DAC.
; When the button is not pressed, should do nothing.
...
start:
  ...
  loop:
    ...
    nobutton: ; If no button pressed, just loop back
  sjmp loop


; Reads ADC, sends to PSOC, gets it back, then writes to DAC
echo:
    lcall readadc
    lcall sndchr
